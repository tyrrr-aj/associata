...
remove_killed_ON(ONG, ONIndex) -> ONG ! {remove_killed_ON, ONIndex}.
...
reset_excitation(ONG) -> ONG ! reset_excitation.
...
process_events(ONs, GlobalCfg, NextONIndex) ->
    receive
        ...
        {get_excitation, Sender} ->
            ONsExcitation = maps:to_list(maps:map(fun(_Index, ON) -> on:get_excitation(ON) end, ONs)),
