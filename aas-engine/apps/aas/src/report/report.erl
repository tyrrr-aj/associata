...
%% start/1 spawns a reporting process with the given config.
%% Expected keys:
%%   mode := pyrlang | silent
%%   structure_id := atom() | binary() | list() | integer()
start(ReportConfig) ->
    spawn(fun() -> process_events(maps:merge(#{mode => pyrlang}, ReportConfig)) end).
